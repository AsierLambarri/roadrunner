...
class TestHDF5ParticleWriter:
    ...
    def test_write_snapshot_creates_datasets(self, tmp_dir):
        ...
        with h5py.File(path, "r") as hf:
            assert "positions" in hf["data"]
            assert "velocities" in hf["data"]
            assert "masses" in hf["data"]
            assert "indices" in hf["data"]
            assert "scaler" in hf["data"]
            assert hf["data/positions"].shape == (50, 3)
            assert hf["data/masses"].shape == (50,)
            assert hf["header"].attrs["time"] == 13.0
            assert hf["header"].attrs["redshift"] == 0.1
    ...
    def test_scaler_round_trip(self, tmp_dir):
        ...
        with h5py.File(path, "r") as hf:
            ...
            scaled_pos = hf["data/positions"][:]
            scaled_vel = hf["data/velocities"][:]

        coords = np.column_stack([snap.position, snap.velocity])
    ...
    def test_multiple_snapshots(self, tmp_dir):
        ...
        with h5py.File(p0, "r") as hf:
            assert hf["data/indices"].shape == (30,)
        with h5py.File(p1, "r") as hf:
            assert hf["data/indices"].shape == (40,)
